...
def parse_nr_header(line):
    ...
    if hdr_set:
        ...
        if nrdb in params.amap:
            func_txt, org_txt = parse_bracket_line(func_org, '[', ']', 1)
            items = [ gi_id, params.amap[nrdb], nrdb_id, func_txt, org_txt ]
    return items

def parse_bracket_line(line, beg_delim, end_delim, rev):    
    in_bracket = ''
    if rev:
    ...
    for i, c in enumerate(line):
        if c == end_delim: end_break += 1
        if c == beg_delim: beg_break += 1
        if beg_break == end_break: break
        if i > 0: in_bracket += c
    if rev:        
        bracket_txt = reverse_string(in_bracket).strip()
        remain_txt  = reverse_string(line[i+1:]).strip()
        return remain_txt, bracket_txt
    else:
        bracket_txt = in_bracket.strip()
        remain_txt  = line[i+1:].strip()
        return bracket_txt, remain_txt

def reverse_string(string):
    return string[::-1]

def get_interpro_map(filename):
    imap = {}
    if filename is None:
        return imap
...
def format_factory(out_files):
    ...
    if form == 'genbank':    
        def parse_genbank(rec):
            ...
            elif (rec.features[0].type == 'source') and ('organism' in rec.features[0].qualifiers):
                org = rec.features[0].qualifiers['organism'][0]
            else:
                return
        ...
        def parse_swiss(rec):
            ...
            if not get_ip:
                ...
                has_ip = 0
                for ref in rec.dbxrefs:
                    ip_m = up_ip_re.match(ref)
                    if ip_m and (ip_m.group(1) in amap):
                        has_ip = 1
                        prot_f.write("\t".join([md5, ip_m.group(1), amap[ip_m.group(1)], rec.annotations['organism'], source]) + "\n")
                if has_ip:
                    seq_f.write("%s\t%s\n" %(md5, seq))                
        return parse_swiss
        ...
        def parse_fasta(rec):
            ...
            if ann_desc == 'greengenes':
                ...
                for x in hdrs:
                    gg_m = gg_re.match(x)
                    if gg_m: tax_list.append(gg_m.group(1))
                    else:    org_list.append(x)
                org  = " ".join(org_list).split(";")[0]
                ...
                if len(hdrs) > 1:
                    ...
                    if ann_desc == 'img':
                        ...
                        if desc[-1] != ']':
                            return
                        func, org = parse_bracket_line(desc, '[', ']', 1)
                    elif ann_desc == 'seed':
                        if desc[-1] != ')':
                            return
                        func, org = parse_bracket_line(desc, '(', ')', 1)
                    elif ann_desc == 'phantome':
                        if (desc[-1] != ']') and (desc[0] != '['):
                            return
                        _junk, org  = parse_bracket_line(desc, '[', ']', 1)
                        func, _junk = parse_bracket_line(desc, '[', ']', 0)
                    elif ann_desc == 'organism':
                        org = desc
                        if source in func_map:
                            func = func_map[source]
                    elif ann_desc == 'function':
                        func = desc
...
def main(args):
    ...
    parser.add_option("-i", "--interpro_map", dest="intermap", metavar="FILE", default=None, help="Optional InterPro names FILE for func mapping with UniProt")
    ...
    elif (params.format == 'swiss') and opts.intermap and os.path.isfile(opts.intermap):
        params.amap = get_interpro_map(opts.intermap)
        params.interpro = True
